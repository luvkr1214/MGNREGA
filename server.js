...
// Example state-district mapping
...
app.get("/api/performance", async (req, res) => {
  ...
  const TTL = 24 * 3600; // 1 day cache

  try {
    const cached = db
      .prepare("SELECT payload, last_fetched FROM cached_districts WHERE state=? AND district=? AND month=?")
      .get(state, district, m);

    if (cached && now - cached.last_fetched < TTL) {
      console.log(`🟢 Served from cache: ${state}, ${district}`);
      return res.json({ source: "cache", data: JSON.parse(cached.payload) });
    }
    ...
    `).run(state, district, m, payload, now);

    console.log(`🔵 Fresh data fetched: ${state}, ${district}`);
    res.json({ source: "gov", data });
  } catch (err) {
    const fallback = db
      .prepare("SELECT payload FROM cached_districts WHERE state=? AND district=? AND month=?")
      .get(state, district, m);

    if (fallback) {
      console.log(`🟠 Served stale cache: ${state}, ${district}`);
      return res.json({
        source: "stale-cache",
